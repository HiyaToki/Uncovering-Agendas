...
for run in ["R1", "R2", "R3"]:
    ...
    t5_agenda_test_file = base_dir + "evaluation/" + run + "/agenda_mlc_t5_test.json"
    mt5_agenda_test_file = base_dir + "evaluation/" + run + "/agenda_mlc_mt5_test.json"
    
    # output file paths for dev set
    t5_agenda_dev_file = base_dir + "evaluation/" + run + "/agenda_mlc_t5_dev.json"
    mt5_agenda_dev_file = base_dir + "evaluation/" + run + "/agenda_mlc_mt5_dev.json"
    
    # trained models file paths
    t5_file_path = base_dir + "/models/" + run + "/AGENDA-MLC-T5/" # classic multi-label classification
    mt5_file_path = base_dir + "/models/" + run + "/AGENDA-MLC-MT5/"
    
    # run prediction for both models
    for model_choice in ["mt5", "t5"]:
