...
for run in ["R1", "R2", "R3"]:
    ...
    en_t5_agenda_test_file = base_dir + "evaluation/" + run + "/agenda_rte_en_t5_test.json"
    bi_mt5_agenda_test_file = base_dir + "evaluation/" + run + "/agenda_rte_bi_mt5_test.json"
    
    mt5_agenda_test_file = base_dir + "evaluation/" + run + "/agenda_mt5_test.json"
    t5_agenda_test_file = base_dir + "evaluation/" + run + "/agenda_t5_test.json"
    
    en_t5_rte_agenda_test_file = base_dir + "evaluation/" + run + "/rte_en_t5_test.json"
    bi_mt5_rte_agenda_test_file = base_dir + "evaluation/" + run + "/rte_bi_mt5_test.json"
    
    # dev files
    en_t5_agenda_dev_file = base_dir + "evaluation/" + run + "/agenda_rte_en_t5_dev.json"
    bi_mt5_agenda_dev_file = base_dir + "evaluation/" + run + "/agenda_rte_bi_mt5_dev.json"
    
    mt5_agenda_dev_file = base_dir + "evaluation/" + run + "/agenda_mt5_dev.json"
    t5_agenda_dev_file = base_dir + "evaluation/" + run + "/agenda_t5_dev.json"
    
    en_t5_rte_agenda_dev_file = base_dir + "evaluation/" + run + "/rte_en_t5_dev.json"
    bi_mt5_rte_agenda_dev_file = base_dir + "evaluation/" + run + "/rte_bi_mt5_dev.json"
    
    # trained models file paths
    en_t5_file_path = base_dir + "models/" + run + "/AGENDA-RTE-EN-T5/" # with pre-train
    bi_mt5_file_path = base_dir + "models/" + run + "/AGENDA-RTE-BI-MT5/"
    
    mt5_file_path = base_dir + "models/" + run + "/AGENDA-MT5/" # no pre-train
    t5_file_path = base_dir + "models/" + run + "/AGENDA-T5/"
    
    en_t5_rte_file_path = base_dir + "models/RTE-EN-T5/" # 0-shot
    bi_mt5_rte_file_path = base_dir + "models/RTE-BI-MT5/"
    
    # run prediction for all models
    for model_choice in ["en_t5", "bi_mt5", "t5", "mt5", "rte_en_t5", "rte_bi_mt5"]:
